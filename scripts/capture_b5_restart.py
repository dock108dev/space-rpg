...
OUT = ROOT / "evidence/B5" / ("restart-capture-" + time.strftime("%Y%m%dT%H%M%SZ", time.gmtime()))
...
passed = False
with tempfile.TemporaryDirectory(prefix="space-opera-b5-visible-") as temporary:
    temp = Path(temporary)
    game = temp / "game"
    ...
    # Match B5's logical canvas in this disposable capture copy only.
    ...
    shutil.copytree(Path(os.environ["B5_RESTART_SOURCE"]) / "synthetic-saves", temp / "saves")
    ...
    env = dict(os.environ, B5_SAVE_DIR=str(temp / "saves"), B5_CAPTURE_DIR=str(OUT))
    arguments = [BINARY, "--path", str(game), "--resolution", "1152x882", "--disable-vsync", "--audio-driver", "Dummy",
                 "--write-movie", str(OUT / "b5-restart.avi"), "--fixed-fps", "30",
                 "--script", "res://tests/visible_restart_b5.gd"]
    ...
    if passed:
        restart = subprocess.run([BINARY, "--headless", "--path", str(game), "--script", "res://tests/restart_b5.gd"], env=env, text=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, timeout=60)
...
if not passed:
    sys.exit(1)
subprocess.run(["ffmpeg", "-v", "error", "-i", str(OUT / "b5-restart.avi"), "-c:v", "libx264",
                "-crf", "20", "-pix_fmt", "yuv420p", "-an", str(OUT / "b5-restart.mp4")], check=True, timeout=150)
metadata = subprocess.check_output(["ffprobe", "-v", "error", "-show_entries",
    "stream=width,height,r_frame_rate,nb_frames,duration:format=duration", "-of", "json", str(OUT / "b5-restart.mp4")], text=True)
