"""B7 connected-world checks with copied runtime and disposable synthetic chapter saves.
...
OUT = ROOT / "evidence/B7" / ("run-" + time.strftime("%Y%m%dT%H%M%SZ", time.gmtime()))
...
results = []
with tempfile.TemporaryDirectory(prefix="space-opera-b7-validation-") as temporary:
    temp = Path(temporary)
    game = temp / "game"
    shutil.copytree(ROOT / "game", game, ignore=shutil.ignore_patterns(".godot"))
    digest = record_runtime(OUT, game)
    env = dict(os.environ, B7_SAVE_DIR=str(temp / "saves"), B7_CHECKS_PATH=str(OUT / "b7-checks.json"), B2_EXTERNAL_LIVE_PID=str(os.getpid()))
    env.update(B6_SAVE_DIR=str(temp / "FORBIDDEN-B6"))
    env.update(B5_SAVE_DIR=str(temp / "FORBIDDEN-B5"))
    # Poison unrelated namespace paths: B7 must never create any of these.
    ...
    stages = [
        ...
        ("b7-journeys-branches-recovery", ["--headless", "--fixed-fps", "60", "--path", str(game), "--script", "res://tests/run_b7.gd"]),
        ("b7-actual-save-and-quit", ["--headless", "--fixed-fps", "60", "--path", str(game), "--script", "res://tests/restart_b7.gd", "--", "--quit-check"]),
        ("b7-separate-process-continue", ["--headless", "--fixed-fps", "60", "--path", str(game), "--script", "res://tests/restart_b7.gd"]),
    ]
    for name, arguments in stages:
        started = time.monotonic()
        try:
            stage_env = dict(env, B2_TEST_NO_QUIT="1") if name in ["b7-journeys-branches-recovery", "b7-presentation-keyboard"] else env
