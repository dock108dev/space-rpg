"""B4 connected-world checks with copied runtime and disposable synthetic chapter saves.
...
OUT = ROOT / "evidence/B4" / ("run-" + time.strftime("%Y%m%dT%H%M%SZ", time.gmtime()))
...
results = []
with tempfile.TemporaryDirectory(prefix="space-opera-b4-validation-") as temporary:
    temp = Path(temporary)
    game = temp / "game"
    shutil.copytree(ROOT / "game", game, ignore=shutil.ignore_patterns(".godot"))
    digest = record_runtime(OUT, game)
    env = dict(os.environ, B4_SAVE_DIR=str(temp / "saves"), B4_CHECKS_PATH=str(OUT / "b4-checks.json"), B2_EXTERNAL_LIVE_PID=str(os.getpid()))
    # Poison unrelated namespace paths: B4 must never create any of these.
    env.update(B3_SAVE_DIR=str(temp / "FORBIDDEN-B3"), B25_SAVE_DIR=str(temp / "FORBIDDEN-B25"), S03_SAVE_DIR=str(temp / "FORBIDDEN-S03"), S04_SAVE_DIR=str(temp / "FORBIDDEN-S04"), B2_SAVE_DIR=str(temp / "FORBIDDEN-B2"))
    (temp / "saves").mkdir()
    stages = [
        ("fresh-import", ["--headless", "--path", str(game), "--import"]),
        ("b4-language-corpus", ["--headless", "--path", str(game), "--script", "res://tests/language_b4.gd"]),
        ("b4-journeys-branches-recovery", ["--headless", "--fixed-fps", "60", "--path", str(game), "--script", "res://tests/run_b4.gd"]),
        ("b4-actual-save-and-quit", ["--headless", "--fixed-fps", "60", "--path", str(game), "--script", "res://tests/restart_b4.gd", "--", "--quit-check"]),
        ("b4-separate-process-continue", ["--headless", "--fixed-fps", "60", "--path", str(game), "--script", "res://tests/restart_b4.gd"]),
    ]
    for name, arguments in stages:
        started = time.monotonic()
        try:
            stage_env = dict(env, B2_TEST_NO_QUIT="1") if name in ["b4-journeys-branches-recovery", "b4-presentation-keyboard"] else env
...
from audit_b4_arithmetic import audit
