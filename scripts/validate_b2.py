...
OUT = ROOT / "evidence/B2" / ("run-" + time.strftime("%Y%m%dT%H%M%SZ", time.gmtime()))
...
results = []
with tempfile.TemporaryDirectory(prefix="space-opera-b2-validation-") as temporary:
    temp = Path(temporary)
    game = temp / "game"
    shutil.copytree(ROOT / "game", game, ignore=shutil.ignore_patterns(".godot"))
    digest = record_runtime(OUT, game)
    env = dict(os.environ, B2_SAVE_DIR=str(temp / "saves"), B2_CHECKS_PATH=str(OUT / "b2-checks.json"), B2_EXTERNAL_LIVE_PID=str(os.getpid()))
    # Poison unrelated namespace paths: B2 must never create any of these.
    env.update(S03_SAVE_DIR=str(temp / "FORBIDDEN-S03"), S04_SAVE_DIR=str(temp / "FORBIDDEN-S04"))
    stages = [
        ("fresh-import", ["--headless", "--path", str(game), "--import"]),
        ("b2-journeys-branches-recovery", ["--headless", "--fixed-fps", "60", "--path", str(game), "--script", "res://tests/run_b2.gd"]),
        ("b2-presentation-keyboard", ["--headless", "--fixed-fps", "60", "--path", str(game), "--script", "res://tests/run_b2_ui.gd"]),
        ("b2-actual-save-and-quit", ["--headless", "--fixed-fps", "60", "--path", str(game), "--script", "res://tests/restart_b2.gd", "--", "--quit-check"]),
        ("b2-separate-process-continue", ["--headless", "--fixed-fps", "60", "--path", str(game), "--script", "res://tests/restart_b2.gd"]),
    ]
