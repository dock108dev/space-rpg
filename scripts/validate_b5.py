"""B5 connected-world checks with copied runtime and disposable synthetic chapter saves.
...
OUT = ROOT / "evidence/B5" / ("run-" + time.strftime("%Y%m%dT%H%M%SZ", time.gmtime()))
...
results = []
with tempfile.TemporaryDirectory(prefix="space-opera-b5-validation-") as temporary:
    temp = Path(temporary)
    game = temp / "game"
    shutil.copytree(ROOT / "game", game, ignore=shutil.ignore_patterns(".godot"))
    digest = record_runtime(OUT, game)
    env = dict(os.environ, B5_SAVE_DIR=str(temp / "saves"), B5_CHECKS_PATH=str(OUT / "b5-checks.json"), B2_EXTERNAL_LIVE_PID=str(os.getpid()))
    # Poison unrelated namespace paths: B5 must never create any of these.
    ...
    stages = [
        ...
        ("b5-language-corpus", ["--headless", "--path", str(game), "--script", "res://tests/language_b5.gd"]),
        ("b5-journeys-branches-recovery", ["--headless", "--fixed-fps", "60", "--path", str(game), "--script", "res://tests/run_b5.gd"]),
        ("b5-actual-save-and-quit", ["--headless", "--fixed-fps", "60", "--path", str(game), "--script", "res://tests/restart_b5.gd", "--", "--quit-check"]),
        ("b5-separate-process-continue", ["--headless", "--fixed-fps", "60", "--path", str(game), "--script", "res://tests/restart_b5.gd"]),
    ]
    for name, arguments in stages:
        started = time.monotonic()
        try:
            stage_env = dict(env, B2_TEST_NO_QUIT="1") if name in ["b5-journeys-branches-recovery", "b5-presentation-keyboard"] else env
...
from audit_b5_arithmetic import audit
