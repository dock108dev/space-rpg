...
OUT = ROOT / "evidence/B3" / ("capture-" + time.strftime("%Y%m%dT%H%M%SZ", time.gmtime()))
...
passed = False
with tempfile.TemporaryDirectory(prefix="space-opera-b3-visible-") as temporary:
    temp = Path(temporary)
    game = temp / "game"
    ...
    # Match B3's logical canvas in this disposable capture copy only.
    ...
    env = dict(os.environ, B3_SAVE_DIR=str(temp / "saves"), B3_CAPTURE_DIR=str(OUT))
    arguments = [BINARY, "--path", str(game), "--resolution", "1152x882", "--disable-vsync", "--audio-driver", "Dummy",
                 "--write-movie", str(OUT / "b3-tour.avi"), "--fixed-fps", "30",
                 "--script", "res://tests/visible_b3.gd"]
    ...
    if passed:
        restart = subprocess.run([BINARY, "--headless", "--path", str(game), "--script", "res://tests/restart_b3.gd"], env=env, text=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, timeout=60)
...
if not passed:
    sys.exit(1)
subprocess.run(["ffmpeg", "-v", "error", "-i", str(OUT / "b3-tour.avi"), "-c:v", "libx264",
                "-crf", "20", "-pix_fmt", "yuv420p", "-an", str(OUT / "b3-tour.mp4")], check=True, timeout=150)
metadata = subprocess.check_output(["ffprobe", "-v", "error", "-show_entries",
    "stream=width,height,r_frame_rate,nb_frames,duration:format=duration", "-of", "json", str(OUT / "b3-tour.mp4")], text=True)
