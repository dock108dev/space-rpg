...
OUT = ROOT / "evidence/B8" / ("capture-" + time.strftime("%Y%m%dT%H%M%SZ", time.gmtime()))
...
with tempfile.TemporaryDirectory(prefix="space-opera-b8-visible-") as temporary:
    ...
    arguments = [BINARY, "--path", str(game), "--resolution", "1152x882", "--disable-vsync", "--audio-driver", "Dummy",
                 "--write-movie", str(OUT / "b8-tour.avi"), "--fixed-fps", "30",
                 "--script", "res://tests/visible_b8.gd"]
...
if not passed:
    sys.exit(1)
subprocess.run(["ffmpeg", "-v", "error", "-i", str(OUT / "b8-tour.avi"), "-c:v", "libx264",
                "-crf", "20", "-pix_fmt", "yuv420p", "-an", str(OUT / "b8-tour.mp4")], check=True, timeout=150)
metadata = subprocess.check_output(["ffprobe", "-v", "error", "-show_entries",
    "stream=width,height,r_frame_rate,nb_frames,duration:format=duration", "-of", "json", str(OUT / "b8-tour.mp4")], text=True)
