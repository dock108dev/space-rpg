...
OUT = ROOT / "evidence/B2.5" / ("capture-" + time.strftime("%Y%m%dT%H%M%SZ", time.gmtime()))
...
passed = False
with tempfile.TemporaryDirectory(prefix="space-opera-b25-visible-") as temporary:
    temp = Path(temporary)
    game = temp / "game"
    ...
    # Match B2.5's logical canvas in this disposable capture copy only.
    ...
    env = dict(os.environ, B25_SAVE_DIR=str(temp / "saves"), B25_CAPTURE_DIR=str(OUT))
    arguments = [BINARY, "--path", str(game), "--resolution", "1152x882", "--disable-vsync", "--audio-driver", "Dummy",
                 "--write-movie", str(OUT / "b25-tour.avi"), "--fixed-fps", "30",
                 "--script", "res://tests/visible_b25.gd"]
...
if not passed:
    sys.exit(1)
subprocess.run(["ffmpeg", "-v", "error", "-i", str(OUT / "b25-tour.avi"), "-c:v", "libx264",
                "-crf", "20", "-pix_fmt", "yuv420p", "-an", str(OUT / "b25-tour.mp4")], check=True, timeout=150)
metadata = subprocess.check_output(["ffprobe", "-v", "error", "-show_entries",
    "stream=width,height,r_frame_rate,nb_frames,duration:format=duration", "-of", "json", str(OUT / "b25-tour.mp4")], text=True)
