"""B8 connected-world checks with copied runtime and disposable synthetic chapter saves.
...
OUT = ROOT / "evidence/B8" / ("run-" + time.strftime("%Y%m%dT%H%M%SZ", time.gmtime()))
...
results = []
with tempfile.TemporaryDirectory(prefix="space-opera-b8-validation-") as temporary:
    temp = Path(temporary)
    game = temp / "game"
    shutil.copytree(ROOT / "game", game, ignore=shutil.ignore_patterns(".godot"))
    digest = record_runtime(OUT, game)
    env = dict(os.environ, B8_SAVE_DIR=str(temp / "saves"), B8_CHECKS_PATH=str(OUT / "b8-checks.json"), B8_ERGONOMICS_PATH=str(OUT / "b8-ergonomics.json"), B2_EXTERNAL_LIVE_PID=str(os.getpid()))
    env.update(B7_SAVE_DIR=str(temp / "FORBIDDEN-B7"))
    env.update(B6_SAVE_DIR=str(temp / "FORBIDDEN-B6"))
    env.update(B5_SAVE_DIR=str(temp / "FORBIDDEN-B5"))
    # Poison unrelated namespace paths: B8 must never create any of these.
    ...
    stages = [
        ...
        ("b7-language-corpus", ["--headless", "--path", str(game), "--script", "res://tests/language_b7.gd"]),
        ("b8-journeys-branches-recovery", ["--headless", "--fixed-fps", "60", "--path", str(game), "--script", "res://tests/run_b8.gd"]),
        ("b8-actual-save-and-quit", ["--headless", "--fixed-fps", "60", "--path", str(game), "--script", "res://tests/restart_b8.gd", "--", "--quit-check"]),
        ("b8-separate-process-continue", ["--headless", "--fixed-fps", "60", "--path", str(game), "--script", "res://tests/restart_b8.gd"]),
        ("b8-ergonomics", ["--headless", "--fixed-fps", "60", "--path", str(game), "--script", "res://tests/ergonomics_b8.gd"]),
        ("b8-ergonomics-separate-process", ["--headless", "--fixed-fps", "60", "--path", str(game), "--script", "res://tests/restart_b8.gd"]),
    ]
    for name, arguments in stages:
        started = time.monotonic()
        try:
            stage_env = dict(env, B2_TEST_NO_QUIT="1") if name in ["b8-journeys-branches-recovery", "b8-ergonomics", "b8-presentation-keyboard"] else env
            if name.startswith("b8-ergonomics"):
                (temp / "saves/ergonomics").mkdir(exist_ok=True)
                stage_env = dict(stage_env, B8_SAVE_DIR=str(temp / "saves/ergonomics"))
