...
Prepared September 23, 2026; reconciled after B2 delivery. **B1 planning and B2 engineering complete; B2.5 engineering complete and owner accepted, B3 and B4 technically complete with owner review pending, then B5–B9. Owner beta acceptance pending.** The owner requested an up-to-date project and the first full slice as a prompt for their lead engineer and team. The original packet prepared that handoff; no game, owner session or application validation was run while writing it. B2's later implementation has its own [delivery evidence](../evidence/B2/README.md).
...
## 1. B1 preparation baseline — historical
...
| B2.5 | [Command adventure](slices/B2.5-command-adventure.md): natural requests, exact movement, contextual choices, narration and a meaningful multi-approach situation on the B2 world | A1/A1a playable together; actual rules/state/outcomes remain authoritative; model choice assessed |
...
Each stage includes its own usable command/choice interactions, story/gameplay consequences, UI, assets, save meaning, error feedback and relevant checks. A1/A1a acceptance applies throughout B3–B10. B3/B6 must extend B2 rather than replace it with unrelated demos. S05 remains a historical prepared prototype review; future owner observations are recorded separately on the build actually played.
...
Use the shared glass UI to support readable narrative/action history, command input, contextual choices and the illustrated scene. A1 command play and A1a story/gameplay take priority over ornamental polish. Review walking, stopping, turning, doorway crowding, depth order, creature preparation and action feedback at gameplay scale. Record representative asset authoring/cleanup/integration time when measurable; do not infer active effort from generation waits or render duration. Existing nonurgent collar and pet-hover feedback remains historical, not permission to ignore new visible defects.
...
B4 reconciliation: the explicit main-route kit grants 20 and all three base items. A redeemed B3 bundle grants 4; access and survey grant 0. One gear + chosen power cost 8, leaving 12; all B4 improvements cost 16, leaving 4. B5–B7 amounts above remain planning only. B4 effects are demonstrated in the repeatable illustrated range. B4 is technically complete; owner acceptance remains pending.

B5 reconciliation: explicit completed-collection settlement claim grants home and 10 material once after package, orientation and kit. Three furnishings cost 6 total and usable storage costs 4. All B4 upgrades plus all B5 purchases leave total 4 without optional tasks (representative route 12). B5 is technically complete; owner acceptance pending. Pet care quantities remain B6 planning, not charged or implemented. See [B5 contract](B5-runtime-contract.md) and [B6 handoff](next-task.md).

## B6 delivery reconciliation — September 24

B6-20260924-01 is technically complete with owner acceptance pending. The bounded approach breach demonstrates danger, autonomous support, cover-fetch training, recoverable injury/downing, rescue and explicit paid/free assisted care. This does not complete B7's full expedition. Actual care costs two carried material per actor; B1's one-material care estimate is superseded. Maximum B4/B5 spending leaves four total material, while repeated exhausted-fund recovery uses two attended free rounds. [Exact contract](B6-runtime-contract.md), [evidence](../evidence/B6/README.md), and [B7 handoff](next-task.md) own the current implementation. Earlier planning quantities remain historical, not future funding.

## B7 delivery reconciliation — September 24

B7-20260924-01 completes the six-place route with the relay yard, warden plus induction crossing, surveyed/no-survey alternatives, physical core and return report. Eight carried material is paid exactly once on filing; no new consumable or task reward. Existing supply/access/survey history is preserved. Shared progression and companion effects apply to actual expedition play. [Exact contract](B7-runtime-contract.md) and [evidence](../evidence/B7/README.md) supersede earlier B7 planning. B8 character setup and full-chapter polish subsequently completed as recorded below; B9 packaging and B10 owner acceptance remain unstarted.

## B8 delivery reconciliation — September 24

B8-20260924-01 delivers character name/appearance, coherent session persistence, contextual onboarding/help/current outcomes, explicit safe dry walking and improved cutout motion/conditions/rescue. [Contract](B8-runtime-contract.md), [review](B8-review-and-balance.md) and [evidence](../evidence/B8/README.md). The full chapter remains integrated; no damage/economy tuning changed. B8 is technically complete, owner acceptance pending; B9 packaging and B10 owner review remain unstarted.
