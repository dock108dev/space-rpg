...
func run() -> void:
	var scene=load("res://scenes/expedition.tscn").instantiate();scene.auto_focus_pause=false;root.add_child(scene);await frames(30)
	scene.save_store.directory=OS.get_environment("B7_SAVE_DIR").path_join("case-1")
	...
	root.get_texture().get_image().save_png(OS.get_environment("B7_CAPTURE_DIR").path_join("restart.png"))
	print("TOUR_FINAL ",JSON.stringify(scene.snapshot()))
	var f:=FileAccess.open(OS.get_environment("B7_SAVE_DIR").path_join("restarts.json"),FileAccess.WRITE);f.store_string(JSON.stringify([{"directory":scene.save_store.directory,"expected":scene.snapshot()}]));f.close()
