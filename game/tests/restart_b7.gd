...
func equivalent(a:Variant,b:Variant) -> bool:
	...
	return a==b
func run() -> void:
	var base:=OS.get_environment("B7_SAVE_DIR")
	...
	var failures:=0
	for item in cases:
		var scene=load("res://scenes/expedition.tscn").instantiate();scene.auto_focus_pause=false;root.add_child(scene);await process_frame
