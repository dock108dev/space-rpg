...
Prepared September 23, 2026. **READY FOR IMPLEMENTATION; NOT STARTED.** B1 planning is complete in the [chapter packet](../chapter-implementation-packet.md). This is the first full implementation handoff for the chapter. Preparation did not run gameplay or supply owner acceptance.
...
Planning examples for new paths are `game/scenes/chapter_opening.tscn`, `scripts/launch_b2.sh`, `Launch B2.command`, `scripts/validate_b2.py` and `evidence/B2/<attempt>/`. They do not exist yet. Add a `B2` case to the existing `scripts/validate.sh` only once the runner exists. Separate modules for location state or party behavior are useful when they remove concrete coupling; avoid an unrelated controller rewrite.
...
Deliver a named launchable B2 candidate, exact commit plus complete dirty-source/art manifest, relevant retained checks, moving review, known issues and a brief ordinary-use guide. Update the README, Desktop tracker, slice board and current handoff. Distinguish implemented/technically verified/visually reviewed/owner-unreviewed. End with B3's concrete next scope and unresolved dependencies; do not automatically start B3 or mark the beta ready.
