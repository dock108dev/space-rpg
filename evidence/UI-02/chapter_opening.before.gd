...
func _ready() -> void:
	...
	message="The shopping concourse is under takeover. Inspect the powers, then choose one. Continue resumes your latest B2 save."

func build_chapter_ui() -> void:
	var ui:=CanvasLayer.new();ui.process_mode=Node.PROCESS_MODE_ALWAYS;add_child(ui)
	for rect in [Rect2(16,8,1248,98),Rect2(16,648,1248,66)]:
	...
	status.position=Vector2(30,17);status.size=Vector2(1210,30);status.add_theme_font_size_override("font_size",23);ui.add_child(status)
	detail.position=Vector2(30,51);detail.size=Vector2(1210,51);detail.add_theme_font_size_override("font_size",16);ui.add_child(detail)
	feedback.position=Vector2(30,677);feedback.size=Vector2(1210,35);feedback.autowrap_mode=TextServer.AUTOWRAP_WORD_SMART;feedback.add_theme_font_size_override("font_size",15);ui.add_child(feedback)
	audience.position=Vector2(30,651);audience.add_theme_font_size_override("font_size",14);audience.modulate=Color("cabfd7");ui.add_child(audience)
	selection_panel.position=Vector2(30,112);selection_panel.add_theme_constant_override("separation",8);ui.add_child(selection_panel)
	...
	play_panel.position=Vector2(30,112);play_panel.add_theme_constant_override("separation",8);ui.add_child(play_panel)
	...
	chapter_panel.position=Vector2(30,112);chapter_panel.add_theme_constant_override("separation",7);ui.add_child(chapter_panel)
	chapter_buttons.interact=add_button(chapter_panel,"Interact · E",interact)
	chapter_buttons.walk=add_button(chapter_panel,"Assign walk",assign_walk)
	...
	chapter_buttons.kit=add_button(chapter_panel,"Use kit",use_kit)
	utility_panel.position=Vector2(30,156);utility_panel.add_theme_constant_override("separation",7);ui.add_child(utility_panel)
	buttons.save=add_button(utility_panel,"Save",manual_save)
	buttons.load=add_button(utility_panel,"Continue latest",load_latest)
	buttons.reset=add_button(utility_panel,"New practice",reset_demo)
	add_button(utility_panel,"Pause / quit · Esc",toggle_pause)
	reward_panel.position=Vector2(580,156);reward_panel.add_theme_constant_override("separation",5);ui.add_child(reward_panel)
	for id in REWARDS:
	...
	recruit_panel.position=Vector2(580,156);recruit_panel.add_theme_constant_override("separation",6);ui.add_child(recruit_panel)
	chapter_buttons.join=add_button(recruit_panel,"Join me",join_recruit)
	chapter_buttons.decline=add_button(recruit_panel,"Travel alone",decline_recruit)
	...
	pause_panel.position=Vector2(366,225);pause_panel.custom_minimum_size=Vector2(548,360);pause_panel.add_theme_stylebox_override("panel",GlassUI.panel_style(true));ui.add_child(pause_panel)
	...
	var heading:=Label.new();heading.text="Paused · B2 opening";heading.add_theme_font_size_override("font_size",26);box.add_child(heading)
	pause_notice.custom_minimum_size=Vector2(495,76);pause_notice.autowrap_mode=TextServer.AUTOWRAP_WORD_SMART;pause_notice.add_theme_font_size_override("font_size",16);box.add_child(pause_notice)
	add_button(box,"Resume · Esc",toggle_pause)
	chapter_buttons.save_quit=add_button(box,"Save and quit",save_and_quit)
	var row:=HBoxContainer.new();row.add_theme_constant_override("separation",8);box.add_child(row)
	add_button(row,"Retry save",manual_save)
	add_button(row,"Continue latest",load_latest)
	add_button(box,"Recover interrupted save access",recover_save_access)
	var note:=Label.new();note.text="New practice keeps earlier snapshots. No progress runs while paused.";note.add_theme_font_size_override("font_size",14);box.add_child(note)
...
func interact() -> bool:
	...
	if distance(player_cell,RECRUIT)<=1:
		message="The traveler offers to walk with you. Choose Join me or Travel alone above; you may change your mind here."
		return true
...
func refresh_ui() -> void:
	...
	var paused:=get_tree().paused;var selection:=phase=="selection";var combat:=journey=="encounter"
	selection_panel.visible=selection;play_panel.visible=combat;chapter_panel.visible=not selection and not combat
	...
	if selection:
		status.text="B2 · A way through the takeover"
		detail.text="Try each power, then choose one. You control the protagonist; the pet follows. No countdown.\n"+power_description(preview)
	elif combat:
		status.text="Assessment · %s    Health %d/6    Creature %d/6    AP %d/4" % [POWER_NAMES.get(power,""),hp,enemy_hp,ap]
		detail.text=("Your turn" if phase=="player" else "Creature turn")+" · "+action_description()+"\nWASD / arrows: step · Click targets · Enter: end turn · Right-click: cancel · Esc: pause"
	else:
		status.text=("Temporary shelter" if location=="shelter" else "Shopping concourse")+"    Health %d/6    Kits %d    %s" % [hp,kits,"Pet + traveler" if recruit_status=="joined" else "Pet companion"]
		var objective:="Reach the package and enter the collection area." if journey=="arrival" else "Collect the cleared package to teach fetch." if journey=="package" else "Enter shelter; claim one reward at the orientation desk." if reward.is_empty() else "Try fetch in the concourse, then return, save and quit." if not cache_taken else "The cache is returned. Head back to shelter, then save and quit." if location=="concourse" else "B2 route complete. Save and quit; Continue resumes this state."
		detail.text=objective+"\nWASD / arrows: step · E: interact · Click adjacent floor: step · Assign walk: safe travel · Esc: pause"
	feedback.text=message
	pause_notice.text=("All actors and actions are frozen. Save and quit is ready." if stable_boundary() else "All actors and actions are frozen. Resume to finish the current action before saving.")+"\n"+message
	chapter_buttons.save_quit.disabled=not stable_boundary()
	audience.text="OUTSIDE AUDIENCE · Player view only; characters cannot see this: "+("“Temporary” is doing a lot of work here." if location=="shelter" else "One package. A surprisingly aggressive collection policy.")
	for id in ["blast","shield","dash"]:
	...
	buttons.save.disabled=not stable_boundary();buttons.load.disabled=busy or phase=="enemy" or not fetch_state.is_empty();buttons.reset.disabled=buttons.load.disabled
	for id in ["interact","walk","fetch","kit"]:chapter_buttons[id].disabled=not safe_idle()
	chapter_buttons.stop.disabled=not routine or paused;chapter_buttons.kit.disabled=not safe_idle() or kits==0 or hp==6
	chapter_buttons.interact.text=("Enter assessment · E" if journey=="arrival" else "Collect package · E" if journey=="package" else "Door / interact · E") if location=="concourse" else "Door / interact · E"
	...
	chapter_buttons.join.visible=recruit_status in ["available","declined"];chapter_buttons.decline.visible=recruit_status in ["available","declined"];chapter_buttons.wait.visible=recruit_status=="joined";chapter_buttons.rejoin.visible=recruit_status in ["waiting","declined"]
