...
out=root/'evidence/S03'/('capture-'+time.strftime('%Y%m%dT%H%M%SZ',time.gmtime()))
...
with tempfile.TemporaryDirectory(prefix='s03-visible-') as temp:
    ...
    env=dict(os.environ,S03_SAVE_DIR=saves,S03_CAPTURE_DIR=str(out))
    # Preserve gameplay-scale 1280x720 window; record actual movie dimensions from engine/ffprobe.
    args=[binary,'--path',str(copy),'--resolution','1280x720','--disable-vsync','--write-movie',str(out/'encounter.avi'),'--fixed-fps','30','--script','res://tests/visible_s03.gd']
    start=time.monotonic()
    with (out/'visible.log').open('w') as log:
        try:result=subprocess.run(args,env=env,stdout=log,stderr=subprocess.STDOUT,timeout=180);code=result.returncode
